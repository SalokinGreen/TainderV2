import pile from "./tokenizerPile";
const range = (x, y) => {
...
class Tokenizer {
  constructor(model) {
    ...
    const encoder = pile;
    this.vocab = encoder.model.vocab;

    if (!this.vocab || !encoder.model.merges)
      throw new Error(
        `Tokenizer couldn't be loaded, missing vocab or bpe merges for file ${file}`
      );

    const decoder = pile;
    Object.keys(this.vocab).map((x) => {
      decoder[this.vocab[x]] = x;
    });

    const bpe_merges = encoder.model.merges.map((x) => {
  ...
  decode(tokens) {
    let text = tokens.map((x) => this.vocab[x]).join("");
